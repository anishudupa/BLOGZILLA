			...
			<div className="flex gap-3 border rounded-sm p-2 border-white w-[900px]">
				<BoldIcon
					className={`hover:cursor-pointer rounded-sm ${
						editor.isActive("bold") ? "is-active bg-slate-500" : ""
					}`}
					onClick={() => editor.chain().focus().toggleBold().run()}
				/>
				<Italic
					className={`hover:cursor-pointer rounded-sm ${
						editor.isActive("italic") ? "is-active bg-slate-500" : ""
					}`}
					onClick={() => editor.chain().focus().toggleItalic().run()}
				/>
				<UnderlineIcon
					className={`hover:cursor-pointer rounded-sm ${
						editor.isActive("underline") ? "is-active bg-slate-500" : ""
					}`}
				...
				<Highlighter
					className={`hover:cursor-pointer rounded-sm ${
						editor.isActive("highlight") ? "is-active bg-slate-500" : ""
					}`}
